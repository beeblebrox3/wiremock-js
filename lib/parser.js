...
Parser.prototype.normalizeRule = function (value) {
    ...
    if (value.hasOwnProperty("equalTo")) {
        return {
...
Parser.prototype.parseResponse = function (responseConfig) {
    ...
    if (responseConfig.headers) {
        response.headers = Object.assign(response.headers, responseConfig.headers);
    }

    if (responseConfig.transformers && Array.isArray(responseConfig.transformers)) {
        response.transformers = responseConfig.transformers;
    }

    if (responseConfig.fault && responseConfig.fault.length) {
        response.fault = responseConfig.fault;
    }
